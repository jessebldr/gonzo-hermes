...
def _build_container_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Build the shared container backend contract from terminal config.

    Terminal, file, and execute-code tools can each be the first caller that
    creates an environment.  Keeping the projection here prevents the first
    tool used from silently changing lifecycle, mounts, egress, or env policy.
    """
    return {
